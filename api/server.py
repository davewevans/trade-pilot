...
def _claude_costs_from_token_usage(
    conn: sqlite3.Connection,
    window: str,
    strategy_types: list[str] | None,
) -> dict:
    """Aggregate cost/token stats from the token_usage table for the given window.

    token_usage is populated on every Claude API call (see
    recorder.record_token_usage), unlike decisions.estimated_cost_usd which no
    production caller sets. This is the live data source for the cost panel.
    """
    if strategy_types is not None and len(strategy_types) == 0:
        return {
            "window": window,
    ...
    # Build time-window predicates (mirrors get_daily_summaries' window filter)
    where_current: list[str] = ["1=1"]
    params_current: list = []
    if days is not None:
        where_current.append("timestamp >= datetime('now', ?)")
        params_current.append(f"-{days} days")

    where_prior: list[str] = ["1=1"]
    params_prior: list = []
    if days is not None:
        where_prior.append("timestamp >= datetime('now', ?)")
        where_prior.append("timestamp < datetime('now', ?)")
    ...
    total_cost = float(cur.get("total_cost_usd") or 0.0)
    prior_cost = float(pri.get("total_cost_usd") or 0.0)
    # decisions_count now counts Claude API calls (one token_usage row per
    # call), which is the correct denominator for a cost panel.
    decisions_count = int(cur.get("decisions_count") or 0)

    # Filled-trades count (calls where the resulting action is a trade, not skip/hold)
    filled_rows = conn.execute(
        f"""
        SELECT COUNT(*) AS n FROM token_usage
        {clause_c} AND UPPER(decision_action) NOT IN ('SKIP', 'HOLD')
        """,
        params_current,
    ).fetchone()
    filled_count = int(filled_rows["n"]) if filled_rows else 0
    ...
    # Cost by outcome bucket (open/close/skip mapped from decision_action vocabulary)
    outcome_rows = conn.execute(
        f"""
        ...
        FROM token_usage
        {clause_c}
        GROUP BY UPPER(decision_action)
        """,
        params_current,
    ).fetchall()
    ...
    for r in outcome_rows:
        act = (r["decision_action"] or "").upper()
        cost_val = float(r["cost_usd"] or 0.0)
        cnt = int(r["cnt"] or 0)
        if act in _OPEN_ACTIONS:
            bucket = "open"
    ...
    model_rows = conn.execute(
        ...
        SELECT model,
               ...
               COALESCE(SUM(estimated_cost_usd), 0) AS cost_usd
        FROM token_usage
        {clause_c} AND model IS NOT NULL
        GROUP BY model
        """,
        params_current,
    ).fetchall()
