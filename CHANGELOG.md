...
### Added
- **Hermes daily evaluation-bundle publisher** (flag-gated, off by default): a new `publish_reports` job builds the day's daily evaluation bundle and commits it to the private reports repo (`GITHUB_REPORTS_REPO`) that the Hermes self-improvement agent reads. New `data/report_publisher.py` wraps the GitHub Contents API — base64 PUT to `daily/<date>.md`, with a 422 → GET-`sha` → PUT-with-`sha` retry so same-day re-runs overwrite cleanly. Publishing is data exhaust: every network call is wrapped (`timeout=20`) and any failure is logged and returns `False` — it never raises into the scheduler loop, so a GitHub outage cannot affect trading. The job is a no-op when `HERMES_PUBLISH_ENABLED` is false or the repo/token are unset, and publishes unconditionally when on (a zero-decision day is itself signal for Hermes's watchdog). Wired into `config.SCHEDULE` as a `weekday` entry at 16:45 ET — mirroring `post_market`'s Mon–Fri ET cadence, 15 minutes after it, since the bundle only has content on trading days. New config: `HERMES_PUBLISH_ENABLED` / `GITHUB_REPORTS_REPO` / `GITHUB_REPORTS_TOKEN` (+ `.env.example`). Backfillable via `python main.py --job publish_reports`. The flag is flipped in Render, not in code; bundle-content enrichment is a separate change that lands before the flag is enabled. Out of scope: weekly/monthly publish tails.
- **Regression tests**: `tests/test_report_publisher.py` (14 tests — flag-off/missing-credential no-ops, create 201, update via 422→sha→200, sha-lookup-failure, network error, timeout, unexpected status, and `publish_reports.run()` short-circuit / build-and-publish / never-raises-on-any-failure; `requests` fully mocked, no real GitHub calls). `tests/test_guardrails_cc_account_isolation.py` (8 tests pinning the covered-call ownership check: the `self.broker` fallback reads the instance's own broker so per-strategy Guardrails verify ownership against the executing account, and the check fails closed when ownership can't be positively confirmed — broker `None`, missing `get_equity_positions`, the call raising, or shares absent).

### Fixed
- **Standard Wheel order misrouting to the wrong account**: `jobs/market_open.py` evaluated the Standard Wheel against `wheel_broker` (paper_2) via `wheel_ctx_builder` but executed the resulting decision through the default `broker` (paper_1 / adaptive spreads) — wheel orders, and at least one assignment (VZ, 100 sh), landed in the spreads account while Claude reasoned about paper_2. Fixed by routing execution through the same account-bound broker used to build each strategy's context: `execute_decision(wheel_broker, decision)`. The two wheels now also validate through per-strategy `Guardrails` (`wheel_guardrails` / `turnover_wheel_guardrails`) bound to their executing broker, so the covered-call equity-ownership fallback reads the correct account. Regression coverage in `tests/test_wheel_execute_account_routing.py` (AST-based wiring assertions: no strategy executes through the default broker; every execution broker matches a ContextBuilder broker; the wheels' Guardrails are bound to their executing broker).
